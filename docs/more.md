...
> 不可以，目前还没有见到有只买一个平板就能够毕业的大学生。不管是平时做 PPT、写论文、选课系统还是学习编程或是配置 AI 环境等，都离不开 X64 架构的 CPU 和 Windows（Linux）系统，而一般平板搭载的大多都是 ARM 架构的 CPU 搭配安卓系统，因此除非你能保证每一次需要使用电脑时都能够从别人那里借到，不然你最好还是购买一个笔记本或安装一台台式机使用。
