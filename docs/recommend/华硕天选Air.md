# 华硕 天选 Air

## 外观

<div style="margin: 0 auto; text-align: center; width: 70%"><img src="./assets/天选air%201.png" /></div>

<div style="margin: 0 auto; text-align: center; width: 70%"><img src="./assets/天选air%202.png" /></div>

## 配置

|   项目   |                     参数                     |
| :------: | :------------------------------------------: |
| 机身参数 |               14 英寸；1.46kg                |
| 核心配置 |    AMD AI 7 H 350、RTX 5060；AMD AI Max+ 392 |
| 存储配置 | 32G LPDDR5X-7500MT/s、32G LPDDR5X-8000MT/s；1T 三星 PM9C1b |
| 屏幕配置 | 2560\*1600；100%sRGB 高色域；165Hz；400nits  |
| USB 接口 | USB-A:10Gbps\*2 ；USB-C:10Gbps\*1、40Gbps\*1 |
| 影音接口 |   HDMI 2.1；3.5mm 音频接口；DP 2.1（独显）   |
| 其它接口 |               Micro SD 读卡器                |
| 供电配置 | 200W DC 电源接口；100W PD 充电；73Wh 锂电池  |
| 网络配置 |              瑞昱 8852CE无线网卡                |

主购买链接：[R7 H 260+RTX5060 白色 32G+1TB ￥ 8047.8 （JD 国补）](https://3.cn/2GRqr-ku?jkl=@S0oNq3sDg7@)

副购买链接：[AI Max+ 392 黑色 32G+1TB ￥ 9999 （JD 国补）](https://3.cn/2G-RqeIT?jkl=@Q4IpA9aOW8@)

## 优缺点[<Icon icon="clarity:info-line" />](/recommend/推荐#优缺点)

|              优点              |        缺点        |
| :----------------------------: | :----------------: |
| 游戏性能不错，重量轻薄便于携带 |    机身做工一般    |
|     续航能力在游戏本中较强     |  屏幕色准表现不佳  |
|     同定位机器性价比最高     | 网卡与噪音表现一般 |

## 适合人群

对预算不那么敏感，对品牌售后有着较高的需求，需要一台尺寸小重量轻续航尚可且带有独显的性能小钢炮，外出需求与大型游戏需求较多。

## 总结
作为2024年发布的一代神机，天选Air在这两年内并没有什么大的改动，但其独特的定位在市场上也几乎没有什么竞争对手，因此似乎也没有什么改动的必要。这块屏幕在2024年还能算得上是够用，但放到2026年只能说是和友商的轻薄本都存在一定差距。由于模具没有改变，机器自然是没有RJ45网口的，同时网卡使用的还是小螃蟹的8852CE，因此这台机器在网络连接上可以说是存在明显的短板。机器的电池也并没有升级，产品序列升级了AMD去年发布的AI Max+ 392处理器，这款处理器搭载的8060S核显在跑分与游戏上能基本持平4060显卡，但其最大的优势是32G内存能够转化为显存使用，对于有轻度AI需求的用户可以说是比较不错的选择。总的来说，这台机器适合那些外出需求较多，需要一台续航尚可的小尺寸小钢炮，同时可能有轻微的AI部署需求，并且不在乎预算的用户。但仍要注意的是，因为机身质量较轻，机器所使用的板材相对更薄，在使用中容易发生形变与划痕，因此在使用中请多注意对机身的保护。
